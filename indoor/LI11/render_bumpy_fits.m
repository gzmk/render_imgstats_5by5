...
for bump = 1:11
    for gloss = 1:11
        for meshCount = 1:22
            ...
            writetable(T,'/scratch/gk925/render_imgstats_5by5/indoor/LI11/bumpy_fitrender_Conditions.txt','Delimiter','\t')
            
            % Set preferences
            setpref('RenderToolbox3', 'workingFolder', '/scratch/gk925/render_imgstats_5by5/indoor/LI11');
            
            % use this scene and condition file.
            parentSceneFile = ['GBMeshD',num2str(bump),'G',num2str(gloss),'LI',num2str(lf),'C',num2str(meshCount),'.dae']
            conditionsFile = 'bumpy_fitrender_Conditions.txt';
            %     mappingsFile = ['bumpy',bump_level,'_5by5_correctCameraDistDefaultMappings.txt'];
            mappingsFile = 'MeshLF11DefaultMappings.txt'
            ...
            hints.recipeName = ['GBMeshD',num2str(bump),'G',num2str(gloss),'LI',num2str(lf),'C',num2str(meshCount),'-' datetime];
            ...
            for renderer = {'Mitsuba'}
                ...
                montageName = sprintf('GBMeshD%sG%sLI%sC%s', num2str(bump), num2str(gloss), num2str(lf), num2str(meshCount));
